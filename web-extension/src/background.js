...
chrome.storage.local.get(
  ["currentModel", "contextChips", "selectedPort"],
  (res) => {
    if (res?.currentModel) {
...
function connectBridge(port = selectedPort) {
  ...
  ws.onmessage = (ev) => {
    try {
      ...
      if (data.type === "CHIPS_LIST") {
        ...
        const totalTokens = contextChips.reduce(
      ...
      try {
        ...
        console.debug("storage.set failed", e);
      }
            ...
            if (tabs[0]?.id) {
              chrome.tabs.sendMessage(
                tabs[0].id,
                {
                  type: "INSERT_TEXT",
                  text:
                    `/* FILE: ${data.filename} (${data.languageId}) */\n` +
                    data.text,
                  auto,
                  model: currentModel,
                },
      ...
      console.debug("Invalid bridge message", e);
    }
  };
  ...
  ws.onerror = (e) => {
    console.debug("Bridge error", e);
  };
}

// Format multiple chips into a nicely formatted text block
function formatChipsForInsert(
  chips,
  model = "default",
...
function scheduleReconnect() {
  ...
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    console.error(
      "Max reconnect attempts reached, will not retry automatically",
    );
    return;
  }
  reconnectAttempts++;
...
async function initializeConnection() {
  ...
  if (instances.length > 0) {
    ...
    const targetPort =
      instances.find((i) => i.port === selectedPort)?.port || instances[0].port;
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg?.type === "INSERT_TEXT") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]?.id)
        chrome.tabs.sendMessage(
          tabs[0].id,
          { type: "INSERT_TEXT", text: msg.text, model: currentModel },
          () => {},
        );
    });
    sendResponse({ ok: true });
  ...
  if (msg?.type === "REQUEST_CONTEXT") {
    ...
    pendingContextRequests.set(requestId, (response) => {
      console.debug(
        "Calling stored callback with response:",
